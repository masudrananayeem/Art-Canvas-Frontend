...
async function request(path: string, opts: {method?: string; body?: any; auth?: boolean} = {}) {
  ...
  let res: Response
  try { res = await fetch(`${API_BASE}${path}`, { method: opts.method || 'GET', headers, body: opts.body === undefined ? undefined : JSON.stringify(opts.body), cache: 'no-store' }) }
...
export const artApi = {
  ...
  updateOrderStatus:(id:string,status:string)=>request(`/api/admin/orders/${id}`,{method:'PATCH',body:{status},auth:true}),
