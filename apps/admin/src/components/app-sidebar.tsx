...
function Nav({items}:{items:any[]}){const pathname=usePathname();const{isMobile,setOpenMobile}=useSidebar();return <SidebarMenu>{items.map((item:any)=>{const I=item.icon;return <SidebarMenuItem key={item.url}><SidebarMenuButton asChild tooltip={item.title} isActive={pathname===item.url||pathname.startsWith(item.url+"/")}><Link href={item.url} onClick={()=>isMobile&&setOpenMobile(false)}><I/><span>{item.title}</span></Link></SidebarMenuButton></SidebarMenuItem>})}</SidebarMenu>}
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>){const{hasPermission}=useAdminAuth();const filteredWorkspace=workspace.filter(x=>hasPermission(x.permission));const filteredAdmin=administration.filter(x=>hasPermission(x.permission));const{isMobile,setOpenMobile}=useSidebar();return <Sidebar {...props}><SidebarHeader><SidebarMenu><SidebarMenuItem><SidebarMenuButton size="lg" asChild><Link href="/admin/dashboard" onClick={()=>isMobile&&setOpenMobile(false)}><div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><Logo size={22} className="text-current"/></div><div className="grid flex-1 text-left text-sm leading-tight"><span className="truncate font-semibold">ArtCanvas</span><span className="truncate text-xs text-muted-foreground">Studio Admin</span></div></Link></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarHeader><SidebarContent><SidebarGroup><SidebarGroupLabel>Workspace</SidebarGroupLabel><Nav items={filteredWorkspace}/></SidebarGroup>{filteredAdmin.length>0&&<SidebarGroup><SidebarGroupLabel>Administration</SidebarGroupLabel><Nav items={filteredAdmin}/></SidebarGroup>}{finance.filter(x=>hasPermission(x.permission)).length>0&&<SidebarGroup><SidebarGroupLabel>Finance & Services</SidebarGroupLabel><Nav items={finance.filter(x=>hasPermission(x.permission))}/></SidebarGroup>}<div className="mt-auto p-3"><SidebarMenu><SidebarMenuItem><SidebarMenuButton asChild tooltip="Open storefront"><a href={process.env.NEXT_PUBLIC_ARTCANVAS_STORE_URL||"http://localhost:5173"} target="_blank" rel="noreferrer"><Store/><span>Open storefront</span><ExternalLink className="ml-auto"/></a></SidebarMenuButton></SidebarMenuItem></SidebarMenu></div></SidebarContent></Sidebar>}
