                    ...
                    {isOpen && (
                      <tr className="border-b border-current/10"><td colSpan={7} className="px-4 pb-5 pt-1">
                        <div className="rounded-xl border border-current/10 p-4 grid md:grid-cols-3 gap-5">
                          ...
                          <div><p className="text-[9px] uppercase tracking-widest opacity-40 mb-2">Items</p><div className="space-y-2">{(order.items || []).map((item, i) => <div key={`${order.id}-${i}`} className="flex justify-between gap-3"><span>{item?.name || "Item"} × {Number(item?.qty || 0)}</span><span className="font-mono">${(Number(item?.price || 0) * Number(item?.qty || 0)).toFixed(2)}</span></div>)}</div><div className="mt-3 pt-3 border-t border-current/10"><p className="text-[9px] uppercase tracking-widest opacity-40 mb-2">Status history</p>{(order.statusHistory || []).map((h, i) => <div key={`${h.status}-${i}`} className="flex justify-between gap-3 py-0.5"><span className="capitalize">{h.status}</span><span className="opacity-45">{h.at ? new Date(h.at).toLocaleString() : "—"}</span></div>)}</div></div>
