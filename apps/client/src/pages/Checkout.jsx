...
export default function Checkout() {
  ...
  const deliveryCharge = Number(paymentSettings?.deliveryCharge || 0);
  ...
  const submit = async (e) => {
    ...
    setPlacing(true);
    try {
      const placed = await checkout(form, paymentMethod, paymentRef.trim(), { payerName: payerName.trim(), productCodes: cart.map((x) => x.productCode || x.id), useCoins });
      setOrder(placed);
    } catch (e) {
      setError(e.message);
    } finally {
      ...
      <PageTransition>
        <main className="px-6 py-24 max-w-lg mx-auto text-center">
          ...
          <p className="text-sm opacity-60 mb-6">
            Thank you, {form.fullName.split(" ")[0]}. Your order total is <span className="font-mono font-semibold">৳{Number(order.total||0).toLocaleString()}</span>, to be paid via{" "}
      ...
      <main className="px-6 pt-10 pb-24">
        <div className="max-w-4xl mx-auto grid lg:grid-cols-[1fr_360px] gap-10">
          <div>
            ...
            <h1 className="font-display italic text-3xl font-bold mb-6">Shipping & payment</h1>{membership?.status === "active" && <div className="mb-5 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-4 py-3 text-xs">ArtCanvas Member · reward coins can be used at checkout. Your coin balance is private.</div>}{paymentSettings && (Number(paymentSettings.deliveryCharge||0)>0 || Number(paymentSettings.returnCharge||0)>0) && <div className="mb-5 rounded-xl border border-current/10 px-4 py-3 text-xs opacity-70">Delivery charge: ৳{Number(paymentSettings.deliveryCharge||0).toLocaleString()} · Return charge: ৳{Number(paymentSettings.returnCharge||0).toLocaleString()}</div>}
            ...
            <form onSubmit={submit} className="space-y-6">
              ...
              {membership?.status === "active" && <label className={`flex items-start gap-3 rounded-xl border p-4 cursor-pointer ${dark?"border-white/10 bg-white/[.03]":"border-black/10 bg-black/[.02]"}`}><input type="checkbox" checked={useCoins} onChange={e=>setUseCoins(e.target.checked)} className="mt-1"/><span><strong className="text-sm">Use my ArtCanvas reward coins</strong><span className="block text-xs opacity-55 mt-1">The backend will securely use eligible coins and calculate the final discount. Your balance is never displayed.</span></span></label>}
              ...
              >
                ...
                {placing ? "Placing order…" : `Place order — ৳${estimatedTotal.toLocaleString()}`}
              </motion.button>
            </form>
          </div>
          ...
          <aside className="border border-current/10 rounded-2xl p-5 h-fit">
            ...
            <div className="space-y-2 pt-3 border-t border-current/10 text-sm">
              ...
              {deliveryCharge > 0 && <div className="flex justify-between"><span>Delivery charge</span><span>৳{deliveryCharge.toLocaleString()}</span></div>}
              <div className="flex justify-between text-base font-semibold pt-2"><span>Total</span><span className="font-mono">৳{estimatedTotal.toLocaleString()}</span></div>
            </div>
